eav.factory("aiMetricsService", function($http, $q, localStorageService, aiService) {
    ...
    var aiMetricsService = {
        ...
        calcProgress: function(aiObjects) {
            ...
            var completedAiArray = _.filter(aiObjects, function(ai) {
                return ai.ACTIONTYPE === "Client Review" || ai.ACTIONTYPE === "Bill";
            });
            var incompleteAiArray = _.filter(aiObjects, function(ai) {
                return ai.ACTIONTYPE !== "Client Review" && ai.ACTIONTYPE !== "Ready to Close" && ai.ACTIONTYPE !== "Schedule";
            });

            var completedPriorityArray = _.pluck(completedAiArray, 'ACTIONPRIORITY');
            completedPriorityArray = _.map(completedPriorityArray, function(num) {
                return scaleAdjust - parseInt(num);
            });
            ...
            incompletePriorityArray = _.map(incompletePriorityArray, function(num) {
                return scaleAdjust - parseInt(num);
            });
            //    console.log("incompletePriorityArray",incompletePriorityArray);
            var completedPriorityTotal = _.reduce(completedPriorityArray, function(prev, next) {
                return prev + next;
            }, 0);
            //    console.log("completedPriorityTotal",completedPriorityTotal);
            var incompletePriorityTotal = _.reduce(incompletePriorityArray, function(prev, next) {
                return prev + next;
            }, 0);
